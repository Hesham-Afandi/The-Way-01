...
export class StorageService {
  ...
  public updateSettings(newSettings: Partial<CenterSettings>, currentUser: User): void {
  ...
  // --- Reset to Demo Data ---
  public resetToDemoData(currentUser: User): void {
    ...
    this.assignments = initialTeacherAssignments;
    ...
    this.teacherPayments = initialTeacherPayments;
    ...
    setStored(STORAGE_KEYS.ROOMS, this.rooms);
    setStored(STORAGE_KEYS.TEACHERS, this.teachers);
    setStored(STORAGE_KEYS.STUDENTS, this.students);
    setStored(STORAGE_KEYS.ASSIGNMENTS, this.assignments);
    setStored(STORAGE_KEYS.CONTRACTS, this.contracts);
    setStored(STORAGE_KEYS.SESSIONS, this.sessions);
    setStored(STORAGE_KEYS.ATTENDANCE, this.attendance);
    setStored(STORAGE_KEYS.PAYMENTS, this.payments);
    setStored(STORAGE_KEYS.TEACHER_PAYMENTS, this.teacherPayments);
  ...
  public updateStudent(id: string, updates: Partial<Student>, currentUser: User): void {
    const prev = this.students.find(s => s.id === id);
    this.students = this.students.map(s => (s.id === id ? { ...s, ...updates } : s));
    setStored(STORAGE_KEYS.STUDENTS, this.students);

    this.addNotification({
      type: 'system',
      title: `تعديل بيانات طالب: ${updates.name || prev?.name || ''}`,
      message: `قام ${currentUser.name} بتحديث بيانات الطالب (${prev?.code || ''}) في السنتر`,
    ...
    this.logAudit(
      ...
      'Student',
      id,
      updates.name || prev?.name || 'طالب',
      `تم تعديل بيانات الطالب ${prev?.name || ''}`
    );
    this.notify();
  }

  public archiveStudent(id: string, currentUser: User): void {
    const st = this.students.find(s => s.id === id);
    if (!st) return;
    this.updateStudent(id, { status: StudentStatus.ARCHIVED }, currentUser);
    this.logAudit(
      currentUser.id,
      currentUser.name,
      AuditAction.ARCHIVE,
  ...
  public deleteStudent(id: string, currentUser: User): void {
    const st = this.students.find(s => s.id === id);
  ...
  public addTeacher(data: Omit<Teacher, 'id' | 'code' | 'joinedDate'>, currentUser: User): Teacher {
    const nextCode = `TCH-${200 + this.teachers.length + 1}`;
    ...
    this.logAudit(
      ...
      newTeacher.id,
      newTeacher.name,
      `تم إضافة مدرس جديد: ${newTeacher.name} (${newTeacher.code})`
    );
    this.notify();
    return newTeacher;
  }

  public updateTeacher(id: string, updates: Partial<Teacher>, currentUser: User): void {
    const prev = this.teachers.find(t => t.id === id);
    ...
    this.logAudit(
      ...
      id,
      prev?.name || 'مدرس',
      `تم تعديل بيانات المدرس ${prev?.name || ''}`
    );
    this.notify();
  }
  ...
  public updateSubject(id: string, updates: Partial<Subject>, currentUser: User): void {
  ...
  public addRoom(data: Omit<Room, 'id'>, currentUser: User): Room {
    ...
    this.logAudit(
      ...
      newRoom.id,
      newRoom.name,
      `تم إضافة قاعة جديدة: ${newRoom.name} - سعة ${newRoom.capacity} طلاب`
    );
    this.notify();
    return newRoom;
  }

  public updateRoom(id: string, updates: Partial<Room>, currentUser: User): void {
    this.rooms = this.rooms.map(r => (r.id === id ? { ...r, ...updates } : r));
    setStored(STORAGE_KEYS.ROOMS, this.rooms);
    this.notify();
  }

  public deleteRoom(id: string, currentUser: User): void {
    const rm = this.rooms.find(r => r.id === id);
    ...
    this.logAudit(
      ...
      id,
      rm?.name || 'قاعة',
      `تم حذف القاعة ${rm?.name || ''}`
    );
    this.notify();
  }

  // --- Teacher Assignments (Student + Subject -> Teacher) ---
  ...
  public assignTeacher(
    ...
    const assignment: TeacherAssignment = {
      id: existingIndex >= 0 ? this.assignments[existingIndex].id : `asg-${Date.now()}`,
      studentId,
      subjectId,
      teacherId,
    ...
    // Make sure subject is also added to student subject list
    if (student && !student.subjectIds.includes(subjectId)) {
      this.updateStudent(studentId, { subjectIds: [...student.subjectIds, subjectId] }, currentUser);
    }
    ...
    this.logAudit(
      ...
      `تم تعيين المدرس ${teacher?.name} للطالب ${student?.name} في مادة ${subject?.name}`
    );
  ...
  public addContract(
    ...
    const contractNumber = `CNT-2025-${count.toString().padStart(3, '0')}`;
      ...
      this.addPayment(
        {
          ...
          notes: `دفعة تعاقد أولى - عقد ${contractNumber}`
        },
        currentUser
      );
    }
    ...
    this.logAudit(
      ...
      newContract.id,
      contractNumber,
      `تم إنشاء عقد جديد (${contractNumber}) للطالب ${student?.name || ''} بقيمة ${newContract.totalPrice} ج.م`
    );
  ...
  public updateContract(id: string, updates: Partial<Contract>, currentUser: User): void {
    this.contracts = this.contracts.map(c => (c.id === id ? { ...c, ...updates } : c));
    setStored(STORAGE_KEYS.CONTRACTS, this.contracts);
    this.refreshDynamicAlerts();
    this.notify();
  }

  public deleteContract(id: string, currentUser: User): void {
    const cnt = this.contracts.find(c => c.id === id);
    ...
    this.logAudit(
      ...
      `تم حذف العقد ${cnt?.contractNumber || ''}`
    );
    this.refreshDynamicAlerts();
    this.notify();
  }
  ...
  public addSession(
    ...
    const recurringGroupId = weeks > 1 ? `rec-${Date.now()}` : undefined;

    // Split date string to avoid timezone parsing surprises
    const [yearStr, monthStr, dayStr] = (data.date || new Date().toISOString().split('T')[0]).split('-');
    const year = parseInt(yearStr, 10) || new Date().getFullYear();
    const month = (parseInt(monthStr, 10) || 1) - 1;
    const day = parseInt(dayStr, 10) || new Date().getDate();

    for (let i = 0; i < weeks; i++) {
      const targetDate = new Date(year, month, day + i * 7);
      const yyyy = targetDate.getFullYear();
      const mm = String(targetDate.getMonth() + 1).padStart(2, '0');
      const dd = String(targetDate.getDate()).padStart(2, '0');
      const dateStr = `${yyyy}-${mm}-${dd}`;

      const count = this.sessions.length + createdSessions.length + 1;
      const sessionCode = `SES-${count.toString().padStart(3, '0')}`;
      const newSession: Session = {
        ...data,
        id: `ses-${Date.now()}-${i}-${Math.random().toString(36).substring(2, 5)}`,
        sessionCode,
        date: dateStr,
        recurringGroupId
      };
    ...
    this.sessions = [...createdSessions, ...this.sessions];
    setStored(STORAGE_KEYS.SESSIONS, this.sessions);
    ...
    if (createdSessions.length > 0) {
      this.logAudit(
        ...
        `تمت جدولة ${createdSessions.length} حصة (${createdSessions[0].sessionCode})`
      );
    }
  ...
  public updateSession(id: string, updates: Partial<Session>, currentUser: User): void {
    const prev = this.sessions.find(s => s.id === id);
    ...
    this.logAudit(
      ...
      id,
      prev?.title || 'حصة',
      `تم تعديل بيانات الحصة ${prev?.sessionCode || ''}`
    );
    this.notify();
  }

  public startSession(sessionId: string, currentUser: User): void {
    this.updateSession(sessionId, { status: SessionStatus.LIVE }, currentUser);
  }

  public deleteSession(sessionId: string, currentUser: User): void {
    ...
    this.logAudit(
      ...
      sessionId,
      ses?.title || 'حصة',
      `تم حذف الحصة ${ses?.sessionCode || ''}`
    );
    this.notify();
  }

  // Completing a session triggers session consumption on contracts
  public completeSession(sessionId: string, currentUser: User): void {
    const session = this.sessions.find(s => s.id === sessionId);
    if (!session) return;

    this.updateSession(sessionId, { status: SessionStatus.COMPLETED }, currentUser);

    // If session counts against student packages:
    if (session.countAgainstStudentSessions) {
      session.studentIds.forEach(studentId => {
        // Find matching active contract for this student and subject
        const contract = this.contracts.find(
          c =>
            c.studentId === studentId &&
            c.subjectIds.includes(session.subjectId) &&
        ...
        if (contract) {
          ...
          const newStatus =
            newUsed >= contract.totalSessions
              ? ContractStatus.COMPLETED
              : contract.totalSessions - newUsed <= 3
              ? ContractStatus.EXPIRING_SOON
              : contract.status;

          this.updateContract(contract.id, { usedSessions: newUsed, status: newStatus }, currentUser);
        }
      });
    }
    ...
    this.logAudit(
      ...
      sessionId,
      session.title,
      `تم إنهاء الحصة (${session.sessionCode}) واحتسابها للمدرس والطلاب`
    );
  ...
  public cancelSession(
    ...
    this.updateSession(
      sessionId,
      {
        ...
        countAgainstStudentSessions: chargeStudents,
        notes: session.notes ? `${session.notes} (سبب الإلغاء: ${reason})` : `سبب الإلغاء: ${reason}`
      },
      currentUser
    );
    ...
    if (chargeStudents) {
      session.studentIds.forEach(studentId => {
        ...
        if (contract) {
          const newUsed = Math.min(contract.totalSessions, contract.usedSessions + 1);
          this.updateContract(contract.id, { usedSessions: newUsed }, currentUser);
        }
      });
    }
    ...
    this.logAudit(
      ...
      sessionId,
      session.title,
      `تم إلغاء الحصة (${session.sessionCode}) - ${reason} - ${
        chargeStudents ? 'تم احتساب الحصة' : 'إلغاء مجاني بدون خصم'
      }`
    );
  }
  ...
  public markAttendance(
    ...
    const record: AttendanceRecord = {
      ...
      checkInTime:
        status === AttendanceStatus.PRESENT || status === AttendanceStatus.LATE
          ? this.attendance[existingIndex]?.checkInTime || timeStr
          : undefined,
      notes: notes !== undefined ? notes : this.attendance[existingIndex]?.notes,
    ...
    // If absent, automatically create notification alert
    if (status === AttendanceStatus.ABSENT && student) {
      this.addNotification({
        type: 'student_absent',
        title: `غياب طالب: ${student.name}`,
        message: `تم تسجيل غياب الطالب "${student.name}" عن حصة "${session?.title || 'الحصة'}" اليوم.`,
        date: session?.date || new Date().toISOString().split('T')[0],
        isRead: false,
  ...
  public processStudentCodeAttendance(
    ...
    const studentTodaySessions = this.sessions.filter(
      s => s.date === todayStr && s.studentIds.includes(student!.id) && s.status !== SessionStatus.CANCELLED
    );
    ...
    // Pick active or nearest upcoming session
    const targetSession =
      studentTodaySessions.find(s => s.status === SessionStatus.LIVE) || studentTodaySessions[0];

    // Record attendance
    const attendance = this.markAttendance(
      targetSession.id,
      student.id,
  ...
  public addPayment(
    ...
    const count = this.payments.length + 5001;
    const receiptNumber = `REC-${count}`;
    ...
    if (data.contractId) {
      ...
      if (contract) {
        ...
        this.updateContract(contract.id, { paidAmount: newPaid }, currentUser);
      }
    }
    ...
    this.logAudit(
      ...
      newPayment.id,
      receiptNumber,
      `تم تحصيل مبلغ ${data.amount} ج.م بسند قبض (${receiptNumber}) من الطالب ${student?.name || ''}`
    );
  ...
  public addTeacherPayment(
    ...
    const count = this.teacherPayments.length + 7001;
    const payoutNumber = `PAY-${count}`;
    ...
    this.teacherPayments = [newPayout, ...this.teacherPayments];
    setStored(STORAGE_KEYS.TEACHER_PAYMENTS, this.teacherPayments);
    ...
    this.logAudit(
      ...
      newPayout.id,
      payoutNumber,
      `تم صرف مستحقات بقيمة ${data.amount} ج.م للمدرس ${teacher?.name || ''}`
    );
  ...
  public addNotification(item: Omit<NotificationItem, 'id'>): void {
  ...
  public getActiveUser(): User | null {
    const stored = getStored<User | null>(STORAGE_KEYS.ACTIVE_USER, null);
    if (stored) {
      const liveUser = this.users.find(u => u.id === stored.id || (u.username && u.username.toLowerCase() === stored.username?.toLowerCase()));
      if (liveUser && liveUser.isActive) return liveUser;
    }
  ...
  public addUser(data: Omit<User, 'id'>, currentUser: User): User {
    ...
    this.addNotification({
      type: 'system',
      title: `موظف جديد: ${newUser.name}`,
      message: `تم إنشاء حساب للموظف ${newUser.name} في قسم (${newUser.department}) باسم مستخدم (${newUser.username})`,
      date: new Date().toISOString().split('T')[0],
      isRead: false,
    ...
    this.logAudit(
      ...
      newUser.id,
      newUser.name,
      `تم إنشاء مستخدم جديد: ${newUser.name} اسم مستخدم (${newUser.username}) تخصص (${newUser.department})`
    );
    this.notify();
    return newUser;
  }

  public updateUser(id: string, updates: Partial<User>, currentUser: User): void {
    ...
    const activeStored = getStored<User | null>(STORAGE_KEYS.ACTIVE_USER, null);
    if (activeStored && (activeStored.id === id || activeStored.username?.toLowerCase() === user.username.toLowerCase())) {
      setStored(STORAGE_KEYS.ACTIVE_USER, updatedUser);
    }
    ...
    this.logAudit(
      ...
      id,
      updatedUser.name,
      `تم تحديث بيانات المستخدم: ${updatedUser.name} (${updatedUser.username})`
    );
    this.notify();
  }

  public deleteUser(id: string, currentUser: User): boolean {
    const user = this.users.find(u => u.id === id);
    if (!user) return false;

    // Prevent deleting the main super admin if it's the last one
    if (user.role === UserRole.SUPER_ADMIN && this.users.filter(u => u.role === UserRole.SUPER_ADMIN).length <= 1) {
      return false;
    }
    ...
    this.logAudit(
      ...
      id,
      user.name,
      `تم حذف حساب المستخدم: ${user.name} (${user.username})`
    );
    this.notify();
    return true;
  }

  public resetUserPassword(id: string, newPass: string, currentUser: User): void {
    this.updateUser(id, { password: newPass }, currentUser);
  }
  ...
  public refreshDynamicAlerts(): void {
    ...
    // Check low remaining sessions
    this.contracts.forEach(contract => {
      const remainingSessions = contract.totalSessions - contract.usedSessions;
      if (contract.status === ContractStatus.ACTIVE && remainingSessions <= this.settings.lowSessionsAlertThreshold) {
        const student = this.students.find(s => s.id === contract.studentId);
        const existing = this.notifications.find(
          n => n.relatedEntityId === contract.id && n.type === 'low_sessions'
        );
        if (!existing && student) {
          this.addNotification({
            type: 'low_sessions',
            title: `تنبيه: رصيد حصص منخفض للطالب ${student.name}`,
            message: `متبقي ${remainingSessions} حصص فقط في عقد (${contract.contractNumber}).`,
            date: todayStr,
            isRead: false,
      ...
      if (
        ...
        diffDays >= 0 &&
        diffDays <= this.settings.contractExpiryAlertDays
      ) {
        const student = this.students.find(s => s.id === contract.studentId);
        const existing = this.notifications.find(
          n => n.relatedEntityId === contract.id && n.type === 'contract_expiring'
        );
        if (!existing && student) {
          this.addNotification({
            type: 'contract_expiring',
            title: `تنبيه: اقتراب انتهاء عقد ${contract.contractNumber}`,
            message: `ينتهي عقد الطالب "${student.name}" خلال ${diffDays} يوم/أيام في تاريخ ${contract.endDate}.`,
            date: todayStr,
            isRead: false,
            relatedEntityId: contract.id,
            relatedEntityType: 'contract',
